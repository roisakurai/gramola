  ...
  export function initResizableColumns() {
    const subheaders = document.querySelectorAll(".queue-subheader");
    subheaders.forEach(subheader => {
      const dividers = subheader.querySelectorAll(".sub-divider");
      const dividerAlbum = dividers[0];
      const dividerTime = dividers[1];
        ...
        const handleStart = (clientX) => {
          ...
          function onMouseMove(moveEvent) {
            ...
            if (newTitleWidth > liveTotalWidth - 150) {
              ...
              const containerWidth = subheader.closest('.ex-right')?.clientWidth || 500;
