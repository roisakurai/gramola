...
export function initPlayer() {
  ...
  function playNext() {
    ...
    loadTrack(currentQueue[currentIndex], currentQueue, currentIndex);
  }
    ...
    currentQueue.forEach((track, index) => {
      ...
      if (playOverlay) {
        playOverlay.addEventListener('click', (e) => {
          e.stopPropagation();
          loadTrack(track, currentQueue, index);
        });
      }
  ...
  function loadTrack(track, queue = [], index = -1, shouldPlay = true) {
    if (!track) return;
    
    audio.src = `assets/songs/${track.file}`;
    audio.load();
    ...
    if (isNewQueue) {
      // Reset originalQueue since we loaded a new queue!
      originalQueue = [];
      
      // If shuffle is active, shuffle the new queue immediately!
    ...
    // Update queue header title
    const queueHeaderTitle = document.querySelector('.queue-header h3');
    if (queueHeaderTitle) {
      queueHeaderTitle.textContent = track.album ? `Next From : ${track.album}` : `Next From : Local Library`;
    }
